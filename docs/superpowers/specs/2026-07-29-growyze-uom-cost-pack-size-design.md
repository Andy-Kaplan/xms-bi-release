...
## Addendum (post-deployment, 2026-07-29)

**Actual blast radius, narrower than §"Scope and blast radius" predicted.** Padel Social and Dirty Sixth figures did **not** move. Both orgs' catalogue items have `size = 1`, so `price / 1 = price` makes the staging division a no-op — Padel 2026-05-11 £587,053.68 unchanged; Dirty Sixth 2026-04-23 £791,220.45 unchanged. Only the two Ibis orgs (Heathrow, Gloucester Road) were materially affected. The line above stating "their existing cost/margin/GP% figures will move substantially downward toward correct values" did not hold in practice; the header comment in `19_invitem_uom_cost_pack_size.sql` has been corrected to match.

**Divisor shipped as `COALESCE(NULLIF(size,0),1)`, not the bare `NULLIF(size,0)` implied by this document's line 122.** This document's own §"Why `COALESCE(NULLIF(size, 0), 1)` and not a bare `NULLIF`" already specifies and justifies the `COALESCE(...,1)` form — the divergence is only in the one later reference under "Scope and blast radius"/history claims, not in the change specification itself.

**A satellite backfill was required beyond the staging fix.** `SAT_INVITEM.UOM_COST` is redefined from *cost per pack* to **cost per `measure` unit**, but `19` alone left 4 inventory items on Ibis Gloucester Road holding their old, un-divided cost after the reload — one of them (EASY PEELERS) alone overstated that org's stock value by £918.04. `21_invitem_uom_cost_backfill.sql` was added during execution to target-repair exactly that stuck population (current-flag `SAT_INVITEM` leaf rows still holding the raw pack price), followed by a reload. Post-backfill: 0 genuinely stuck rows across all 4 orgs. See `docs/outstanding/O5-growyze-default-dashboards.md` for the full account of why the reload alone did not reach these rows — the cause is not yet established (see below).

**Line ~122's history claim does not hold for this entity.** This document states: "`SAT_INVITEM` is SCD Type 2 and `UOM_COST` participates in the CDC hash, so corrected costs land as **new satellite rows**. Existing history is preserved, not rewritten." `SAT_INVITEM`'s entity mapping sets `type2_columns = NULL` (`16_invitem_mapping_hotfix.sql`), so every change to this satellite classifies as `T1`, and `sp_ProcessHubSat` handles `T1` by **DELETE-then-INSERT of the current satellite row**, not by closing a version and opening a new one. The platform already rewrites `SAT_INVITEM`'s current row in place on every change — it does not preserve prior versions the way a true Type 2 satellite would. This does not change the safety conclusion (the backfill only rewrites rows already known to be wrong), but the "history preserved" framing above is inaccurate and should not be relied on.

**On why the reload didn't correct the 4 stuck rows:** this remains under investigation, not established. What is confirmed is the symptom (4 Ibis Gloucester Road rows kept their old per-pack cost through a reload) and the remedy (the backfill). What is *not* confirmed is the cause — candidate explanations include a `CHECKSUM` collision in `sp_GenerateCDC`, those items being absent from `load.INVITEM` on the affected run, or something else in the CDC path. The claim that "`UOM_COST` does not participate in CDC change detection" — previously recorded in this branch's artifacts as the root cause — is contradicted by the code: `EntityMappings.entity_columns` for this entity includes `UOM_COST`, and `sp_GenerateCDC` hashes every column in that list except `%_HUB_ID]`/`[LINK_ID]`. See `21_invitem_uom_cost_backfill.sql`'s header, `QUERY_STATUS.md`, and O5 for the corrected, non-committal wording.
