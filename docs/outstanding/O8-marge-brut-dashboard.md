...
| | |
|---|---|
| **Status** | OPEN — mock is live on Oak & Vine; **mock→live conversion** built (8 scripts, branch `worktree-margebrut-live`), gated on fetcher feed + new UAT org (O5 `UOM_COST` gate cleared 2026-07-29) |
...
- **2026-07-29 (gate (b) cleared)** — The O5 `UOM_COST` pack-vs-unit inflation fix (16-113× overstatement of Growyze inventory cost) is deployed and verified on UAT. Marge Brut's consumption-side figures — which this dashboard's hero `Consumption ÷ Turnover` ratio depends on — are now built on trustworthy Growyze cost data. Detail: [O5](O5-growyze-default-dashboards.md) progress log 2026-07-29. Gates (a) fetcher feed and (c) new UAT org creation remain open; the stocktake-cadence limitation noted in Task 6's shape-testing (single `COUNT_DATE` on Ibis Gloucester Road, so month opening/closing consumption can't yet be computed — resolves once a 60-day load lands) is the remaining data dependency once the org exists.
