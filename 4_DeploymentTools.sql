...
-- =====================================================================================
-- STEP 1: CREATE CONFIGURATION TABLE FOR OBJECT DEFINITIONS
-- =====================================================================================

IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'DeploymentObjects' AND schema_id = SCHEMA_ID('core'))
BEGIN
    CREATE TABLE [core].[DeploymentObjects] (
        [ObjectID] int IDENTITY(1,1) NOT NULL,
        [ObjectName] nvarchar(128) NOT NULL,
        [ObjectType] varchar(20) NOT NULL, -- TABLE, FUNCTION, PROCEDURE, INDEX, SAMPLE_DATA
        ...
        CONSTRAINT [PK_DeploymentObjects] PRIMARY KEY CLUSTERED ([ObjectID]),
        CONSTRAINT [UK_DeploymentObjects_Name_Type] UNIQUE ([ObjectName], [ObjectType]),
        CONSTRAINT [CK_DeploymentObjects_Type] CHECK ([ObjectType] IN ('TABLE', 'FUNCTION', 'PROCEDURE', 'INDEX', 'SAMPLE_DATA'))
    );
    
    CREATE NONCLUSTERED INDEX [IX_DeploymentObjects_Order] ON [core].[DeploymentObjects] ([ExecutionOrder], [IsActive]);
    
    PRINT 'Created DeploymentObjects configuration table';
END
GO
---- =====================================================================================
...
CREATE OR ALTER PROCEDURE [core].[sp_DeployObjects]
...
PRINT '';
PRINT '✅ Smart deployment system is ready!';
PRINT '';
PRINT 'USAGE:';
PRINT '  -- Deploy everything:';
PRINT '  EXEC [core].[sp_CreateGlobalParametersTools] @DatabaseName = ''MyDatabase'';';
PRINT '';
PRINT '  -- Deploy only tables and functions:';
PRINT '  EXEC [core].[sp_CreateGlobalParametersTools] @DatabaseName = ''MyDatabase'', @ObjectTypes = ''TABLE,FUNCTION'';';
PRINT '';
PRINT '  -- Deploy with cleanup:';
PRINT '  EXEC [core].[sp_CreateGlobalParametersTools] @DatabaseName = ''MyDatabase'', @DropExisting = 1;';
PRINT '';
PRINT 'To modify objects, update the [core].[DeploymentObjects] table and re-run the deployment.';