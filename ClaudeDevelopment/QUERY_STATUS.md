...
#### DEPLOYMENT UPDATE — 2026-07-30 (later): first UI review — 3 card defects + a dead date filter, all 4 actioned

Ledger [O8](../docs/outstanding/O8-marge-brut-dashboard.md). First look at the rendered dashboard on The Oak & Vine surfaced three cards needing work, and a follow-up report that the date filter did nothing. All four were root-caused to source; three new scripts (`16`, `17`, `18`) plus the O23 fix were deployed the same day. **The dashboard is now 8 cards, not 9, and is date-filterable for the first time.**
...
- `live/18_date_filter_parameter_mappings.sql` — **DEPLOYED to UAT `core` + verified end-to-end against the live card procedures.** The dashboard's date picker did nothing: every card kept showing all months. **The date filter had been wired to the wrong one of the platform's two filter channels.** `core.BuildDynamicWhereClause` reads the date columns from **`ParameterMappings`** (`JSON_VALUE(...,'$.StartDate')` / `'$.EndDate'`) and only emits a predicate when that lookup returns a column; all 9 `MargeBrut*` datasets shipped with **`ParameterMappings = '{}'`**, so `@StartDate`/`@EndDate` were silently discarded — no error, no empty card, just an unfiltered dashboard. `14` had instead invented a `FilterDefinitions` key `PeriodMonth`, which exists on these 9 datasets and **nowhere else in the platform** (enumerated all 35 distinct filter keys across every LIVE record — not one other is date-typed), so nothing ever emitted it; an `IN`-list is also the wrong shape for a range picker. Left inert rather than removed. **The obvious mapping would have been wrong:** `PERIOD_MONTH` is always the 1st, so mapping both slots to it drops the opening month of any mid-month range (15 Jun→15 Jul: naive = 6 rows/£15,078.11/cost NULL, **June gone**; overlap = 12 rows/£32,216.82/27.3%) — and since both agree on whole months it would have passed a casual test, the same normalisation bug already fixed once in `99_verify` check 3. Mapped instead to the month-overlap test `EOMONTH(PERIOD_MONTH) >= @StartDate AND PERIOD_MONTH <= @EndDate` (mapped values are concatenated verbatim, so expressions need no engine change). **The supplier card carried a live `datetime2` trap:** `F_PURCHASES_DAY.ORDER_DATE` is `datetime2`, so mapping `EndDate` to the raw column compares against midnight and drops the whole last day — **16 lines / £129.45** for June, the exact figure already on record for this table; both slots mapped to month-derived (time-free) DATE expressions, which also keeps the card month-aligned with the other seven. Also strips the literal `@FilterClause` from two SQL **comments** (from `14` and `16`) — the card SPs inject via `REPLACE`, which substitutes inside comments too; harmless only while the clause was always empty, which this change ends. **Verified by executing the real card SPs:** clause `AND EOMONTH(F.[PERIOD_MONTH]) >= '2026-06-01' AND F.[PERIOD_MONTH] <= '2026-06-30'`; Turnover £55,221.02 → **£17,138.71**, Cost of Sales 14.8% → **27.3%**, supplier £22,544.37 → **£2,652.84** (bars and header both respond), grid GRAND TOTAL now June; **all 7 remaining card/dataset pairs execute cleanly under a non-empty `@FilterClause` for the first time**.

**Resolved by the date fix — the "two Purchases figures" gap was mostly an artefact of the dead date filter.** Per-month on The Oak & Vine: Feb–Jun 2025 + Feb–Mar 2026 hold **£13,326.32** of PO lines with *no* `F_MARGEBRUT_MONTH` row at all (the fact only covers Apr–Jul 2026), so unfiltered the supplier card summed ~17 months against the grid's 4. **June reconciles to £0.00** (£2,652.84 both sides), confirming these are the *same* measure — so the standing "retitle to Supplier Spend" option is unnecessary. Residual on Apr (+£245.79), May (+£446.44) and Jul (+£468.85) is the stocktake-window effect: the fact's `PURCHASES` is bounded by the first/last count per group-month while PO lines span the whole calendar month, and Apr is the feed's first count while July is incomplete (data ends 24 Jul, last stocktake 30 Jun). **Rule: only fully stocktake-bounded months reconcile — exactly the existing "quote June only" guidance.** `99_verify` check 4 should be re-read with a June window, not unfiltered.
...
> **Updated 2026-07-31 for per-org sales source precedence** (see entry 99). The single-source `product.[BOTTOM_SRC]` scope above is now resolved per org, not assumed to be `int_growyze001`: every POS integration mapped to the org wins, Growyze is the fallback. **Padel and Dirty Sixth (regression gate) unchanged to the penny** — figures as above (Padel), and Dirty Sixth £317,414.99 profit / £405,110.80 net / 78.4% / 12,906 rows. **Oak & Vine (`int_ncraloha001` + `int_mews001`)** now resolves instead of returning empty: £905,503.87 profit / £1,092,561.20 net / 82.9% / 64,724 rows. **Ibis Gloucester resolves to `int_mews001` but the KPI cards render blank** — `AVG_NET_COST` is 100% NULL on every Mews row (703/703); a new cost-path gap, not O7 (see O5 progress log 2026-07-31). **Coverage-mismatch fix folded in:** `GrowyzeProfitPct` previously computed `SUM(PROFIT)/SUM(NET_VALUE)` over mismatched populations (`SUM(PROFIT)` NULL-skips, `SUM(NET_VALUE)` doesn't), reading 63.7% on Oak & Vine instead of 82.9%. Fixed with `AND F.AVG_NET_COST IS NOT NULL` on all four templates — inert on Padel/Dirty Sixth (zero null-cost rows) until precedence widened scope to POS sources. The £ figures were never inflated, only the ratio.
...
> **Updated 2026-07-31 for per-org sales source precedence** (see entry 99). The hardcoded `F.SRC = 'int_growyze001'` filter is now the resolver CTE (every mapped POS integration wins, Growyze is the fallback), and category grain moved `MIDDLE_1`→`TOP` per the O5 portability rule. **Padel and Dirty Sixth unchanged** (regression gate held). **Oak & Vine's pie now shows NCRAloha Food/Drinks plus the full Mews category set** (was Growyze-only, effectively empty since Oak & Vine carries no Growyze sales). **Ibis Gloucester and Ibis Heathrow both resolve to `int_mews001`**: Gloucester's pie shows the Mews category set at £33,144.54 sales; Heathrow is legitimately empty (Mews mapped, no data landed yet). **Open data-quality flag:** at Oak & Vine, `BREAKFAST ADJUSTMENT` accounts for £42,223.26 of £55,221.06 Mews PROD sales (76%) — consistent with what Marge Brut already reports, not a regression, but worth a decision. **Deferred minor:** "Tips" renders as a zero-value pie slice on both Mews orgs (a single £0.04 line, the same rogue value O8 excluded from Marge Brut verification) — `ROUND(...,0)` displays it as a slice with a legend entry.
...
### 99. `integrations/Growyze/reporting_queries/41_verify_sales_precedence.sql`

**Purpose:** New read-only per-org verifier for the sales source precedence rule (94/95): every POS integration mapped to the org wins, Growyze is the fallback, no match renders empty. Five sections, all sharing the same `org_pos`/`sales_src` resolver CTE: **A** — what the resolver picks for this org (resolved source(s) + tier); **B** — sales the cards will report, grouped by `SRC` off `F_LINEITEM_15MIN`; **C** — the margin measures behind `GrowyzeProfit`/`GrowyzeProfitPct`, joined via `product.[BOTTOM_SRC]` with the `AVG_NET_COST IS NOT NULL` coverage guard; **D** — the category set the pie will show, off `F_LINEITEM_15MIN` grouped by `COALESCE(TOP_MICROSERVICE_NAME, TOP_NAME)`; **E** — a negative check reporting both the raw Growyze-PROD row count in the fact (no resolver join) and the count surviving the resolver join, with a verdict of `PASS`/`FAIL`/`VACUOUS`/`N/A` depending on org tier.
...
## Provisioning, Mews rollout and report-DB palettes (versioned 2026-08-04)

These five entries were written as the work was done but lived only on the working tree until the
2026-08-04 branch consolidation. Renumbered to 123-127 on merge: their original numbers
(85a, 92, 94, 95, 96) already belong to other entries on this line.

*Originally filed under: Mews (int_mews001)*

### 123. `integrations/Mews/91_deploy_mews_uat.ps1`
...
*Originally filed under: Prod v1.0 Baseline Validation Kit (2026-07-06)*

### 124. `ibis-provisioning/01–04` (+01b) + `DEPLOY.txt` (O19)
...
*Originally filed under: Oak & Vine → Growyze + Mews Mapping (2026-07-29, ledger O24 / Integrations O10)*

### 125. `oak-vine-mapping/01–02` + `90_deploy_oak_vine_mapping.ps1`
...
### 126. `microservice-report/02_growyze_palette_additional_orgs.sql`
...
### 127. `microservice-report/03_growyze_extended_palette.sql`
