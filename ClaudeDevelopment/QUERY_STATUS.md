...
| `live/15_report_config.sql` | not deployed | still not deployed; `@OrgId`/`@DbPrefix` now **pre-set** to Gloucester |
| `live/99_verify.sql` | not deployed | still not deployed |
...
**Outstanding, blocked externally:** the session's egress IP changed and both UAT servers began refusing connections (report DB explicitly firewall-rejected `94.15.235.51`; the MI public endpoint timed out). Left to do: allow the IP, then run the **`Supplier Dimension` step only** on Gloucester (`presentation.D_SUPPLIER` holds just the `Unknown` sentinel although `datavault.HUB_SUPPLIER` has 8 rows, and it matches **0** of `F_PURCHASES_DAY`'s 3 supplier keys — so `MargeBrutPurchasesBySupplier` returns no data rows; a Growyze/presentation gap, not a Marge Brut defect), then `15`, then `99_verify` + a UI check.
