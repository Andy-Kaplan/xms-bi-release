/* =============================================================================
   03_margebrut_report_config.sql
   -----------------------------------------------------------------------------
   Target server  : xms-mssql-ne-uat   (Azure SQL)
   Target database: report
   Purpose        : Wire the "Marge Brut" dashboard for the Oak & Vine org into the
                    microservice report DB so it renders in the UI.

   Run this DIRECTLY against the report DB (SSMS / azure-data-studio). The MCP
   tools cannot reach it (they land in master; Azure SQL blocks cross-DB queries).

   Layout (single grid, 12-col, hybrid):
     Row 1  KPI strip  : Cost of Sales | Consumption | Turnover | Purchases   (md=3 each)
     Row 2  centrepiece: Marge Brut grid                                       (md=12)
     Row 3            : Cost % by group | Consumption mix                      (md=6 / md=6)
     Row 4            : Comps & staff   | Purchases by supplier                (md=6 / md=6)
   ...
   @OrgId / @DbPrefix are pre-set to The Oak & Vine (existing UAT org). No org-creation step.
   ============================================================================= */
...
DECLARE @OrgId    UNIQUEIDENTIFIER = '7ED2E768-0D22-F111-832F-000D3AB27D87';  -- The Oak & Vine org GUID (OrgID 16)
DECLARE @DbPrefix NVARCHAR(8)      = N'20260317';                              -- The Oak & Vine MI DB prefix
...
BEGIN
    ...
    PRINT 'BiConfig: inserted Oak & Vine';
END
ELSE PRINT 'BiConfig: Oak & Vine already exists, skipped';

----------------------------------------------------------------------
-- 2. VisualisationConfig -- grant the 4 card types Oak & Vine uses
----------------------------------------------------------------------
...
----------------------------------------------------------------------
-- 7. DashboardGroup -- root "All Dashboards" group for Oak & Vine (LOAD-BEARING)
...
PRINT 'DashboardGroup: ensured All Dashboards group for Oak & Vine';
...
PRINT 'OrganisationDashboardGroupMapping: mapped Oak & Vine dashboards into group';

----------------------------------------------------------------------
-- 9. Verification -- every live Oak & Vine config must resolve through a group
----------------------------------------------------------------------
...
IF @unmapped > 0
BEGIN
    PRINT N'ABORT -- ' + CAST(@unmapped AS NVARCHAR(10)) + N' Oak & Vine dashboards are ungrouped. Rolling back.';
    ROLLBACK TRANSACTION; RETURN;
END

COMMIT TRANSACTION;
PRINT '=== Oak & Vine Marge Brut dashboard wired and visible ===';
