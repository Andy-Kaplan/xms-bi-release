-- =============================================================================
-- SurveyHero Integration: StagingControl Records
-- Schema: int_surveyhero001
-- Source: XMS BI DEV (2026-03-13)
-- Total Steps: 7 (3 Staging + 4 Load)
-- =============================================================================
-- Deploys all StagingControl records for the SurveyHero integration.
-- Includes the ANSWER_ID CASE fix in SH_MAIN (stable composite key for
-- unstructured answers instead of raw answer text).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Step 1: Element Mapping Table (Tier 1, Staging)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Step 2: Survey Hero Main (Tier 2, Staging)
-- NOTE: Contains ANSWER_ID CASE fix — uses response_id-element_id composite
-- key for unstructured answers instead of raw answer text
-- -----------------------------------------------------------------------------
...
BEGIN
    ...
    ([step_name], [staging_table], [query_sql], [tier], [step_type], [exclude],
     [description], [depends_on_steps], [retry_count], [timeout_minutes], [staging_columns], [created_at], [updated_at])
    VALUES (N'Survey Hero Main', N'SH_MAIN', N'-- SurveyHero Main staging query
-- ANSWER_ID fix: uses response_id-element_id for unstructured answers
-- instead of raw answer text (prevents truncation and hash instability)
...
-- -----------------------------------------------------------------------------
-- Step 3: Survey Hero Questions (Tier 2, Staging)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- LOAD STEPS (Tier 1) — Data Vault load queries
-- These are new steps not in the original release scripts.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Step 4: Data Vault load - ANSWER (Tier 1, Load)
-- -----------------------------------------------------------------------------
IF EXISTS (SELECT 1 FROM [core].[int_surveyhero001].[StagingControl] WHERE [step_name] = N'Data Vault load - ANSWER')
BEGIN
    UPDATE [core].[int_surveyhero001].[StagingControl]
    SET [staging_table] = N'ANSWER',
        [query_sql] = N'INSERT INTO [load].ANSWER (HUB_ID, ANSWER, BOTTOM_LEVEL, LEVEL_NAME, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC) SELECT HUB_ID, ANSWER, BOTTOM_LEVEL, LEVEL_NAME, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC FROM (SELECT HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', ANSWER_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) AS HUB_ID, ANSWER AS ANSWER, BOTTOM_LEVEL AS BOTTOM_LEVEL, ANSWER_LEVEL_NAME AS LEVEL_NAME, CAST(GETDATE() AS DATE) AS EFFECTIVEFROM, 1 AS CURRENT_FLAG, 0 AS IS_DELETED, GETDATE() AS LOAD_TS, ''int_surveyhero001'' AS SRC, ROW_NUMBER() OVER (PARTITION BY HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', ANSWER_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) ORDER BY (SELECT NULL)) AS rn FROM stage.SH_MAIN) sub WHERE rn = 1',
        [tier] = 1,
        [step_type] = N'Load',
        [exclude] = 0,
...
BEGIN
    ...
    ([step_name], [staging_table], [query_sql], [tier], [step_type], [exclude],
     [description], [depends_on_steps], [retry_count], [timeout_minutes], [staging_columns], [created_at], [updated_at])
    VALUES (N'Data Vault load - ANSWER', N'ANSWER', N'INSERT INTO [load].ANSWER (HUB_ID, ANSWER, BOTTOM_LEVEL, LEVEL_NAME, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC) SELECT HUB_ID, ANSWER, BOTTOM_LEVEL, LEVEL_NAME, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC FROM (SELECT HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', ANSWER_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) AS HUB_ID, ANSWER AS ANSWER, BOTTOM_LEVEL AS BOTTOM_LEVEL, ANSWER_LEVEL_NAME AS LEVEL_NAME, CAST(GETDATE() AS DATE) AS EFFECTIVEFROM, 1 AS CURRENT_FLAG, 0 AS IS_DELETED, GETDATE() AS LOAD_TS, ''int_surveyhero001'' AS SRC, ROW_NUMBER() OVER (PARTITION BY HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', ANSWER_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) ORDER BY (SELECT NULL)) AS rn FROM stage.SH_MAIN) sub WHERE rn = 1', 1, N'Load', 0, N'Data Vault load step for ANSWER', NULL, 3, 30, N'[]', GETDATE(), GETDATE());
END
GO

-- -----------------------------------------------------------------------------
-- Step 5: Data Vault load - TOUCHPOINT (Tier 1, Load)
-- -----------------------------------------------------------------------------
IF EXISTS (SELECT 1 FROM [core].[int_surveyhero001].[StagingControl] WHERE [step_name] = N'Data Vault load - TOUCHPOINT')
BEGIN
    UPDATE [core].[int_surveyhero001].[StagingControl]
    SET [staging_table] = N'TOUCHPOINT',
        [query_sql] = N'INSERT INTO [load].TOUCHPOINT (HUB_ID, TOUCHPOINT_TYPE, TOUCHPOINT_DATETIME, TOUCHPOINT_STATUS, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC) SELECT HUB_ID, TOUCHPOINT_TYPE, TOUCHPOINT_DATETIME, TOUCHPOINT_STATUS, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC FROM (SELECT HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', TOUCHPOINT_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) AS HUB_ID, TOUCHPOINT_TYPE AS TOUCHPOINT_TYPE, TOUCHPOINT_DATE AS TOUCHPOINT_DATETIME, TOUCHPOINT_STATUS AS TOUCHPOINT_STATUS, CAST(GETDATE() AS DATE) AS EFFECTIVEFROM, 1 AS CURRENT_FLAG, 0 AS IS_DELETED, GETDATE() AS LOAD_TS, ''int_surveyhero001'' AS SRC, ROW_NUMBER() OVER (PARTITION BY HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', TOUCHPOINT_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) ORDER BY (SELECT NULL)) AS rn FROM stage.SH_MAIN) sub WHERE rn = 1',
        [tier] = 1,
        [step_type] = N'Load',
        [exclude] = 0,
...
BEGIN
    ...
    ([step_name], [staging_table], [query_sql], [tier], [step_type], [exclude],
     [description], [depends_on_steps], [retry_count], [timeout_minutes], [staging_columns], [created_at], [updated_at])
    VALUES (N'Data Vault load - QUESTION', N'QUESTION', N'INSERT INTO [load].QUESTION (HUB_ID, PARENT_ID, QUESTION, LEVEL_NAME, BOTTOM_LEVEL, QUESTION_ID, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC) SELECT HUB_ID, PARENT_ID, QUESTION, LEVEL_NAME, BOTTOM_LEVEL, QUESTION_ID, EFFECTIVEFROM, CURRENT_FLAG, IS_DELETED, LOAD_TS, SRC FROM (SELECT HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', QUESTION_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) AS HUB_ID, parent_element_id AS PARENT_ID, question_text AS QUESTION, LEVEL_NAME AS LEVEL_NAME, BOTTOM_LEVEL AS BOTTOM_LEVEL, QUESTION_ID AS QUESTION_ID, CAST(GETDATE() AS DATE) AS EFFECTIVEFROM, 1 AS CURRENT_FLAG, 0 AS IS_DELETED, GETDATE() AS LOAD_TS, ''int_surveyhero001'' AS SRC, ROW_NUMBER() OVER (PARTITION BY HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', QUESTION_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) ORDER BY (SELECT NULL)) AS rn FROM stage.SH_QUESTIONS) sub WHERE rn = 1', 1, N'Load', 0, N'Data Vault load step for QUESTION', NULL, 3, 30, N'[]', GETDATE(), GETDATE());
END
GO

-- -----------------------------------------------------------------------------
-- Step 7: Data Vault load - ANSWER_QUESTION_TOUCHPOINT (Tier 1, Load)
-- -----------------------------------------------------------------------------
...
BEGIN
    UPDATE [core].[int_surveyhero001].[StagingControl]
    SET [staging_table] = N'ANSWER_QUESTION_TOUCHPOINT',
...
BEGIN
    ...
    ([step_name], [staging_table], [query_sql], [tier], [step_type], [exclude],
     [description], [depends_on_steps], [retry_count], [timeout_minutes], [staging_columns], [created_at], [updated_at])
    VALUES (N'Data Vault load - ANSWER_QUESTION_TOUCHPOINT', N'ANSWER_QUESTION_TOUCHPOINT', N'INSERT INTO [load].ANSWER_QUESTION_TOUCHPOINT (LNK_ID, TOUCHPOINT_HUB_ID, QUESTION_HUB_ID, ANSWER_HUB_ID, LOAD_TS, SRC) SELECT LNK_ID, TOUCHPOINT_HUB_ID, QUESTION_HUB_ID, ANSWER_HUB_ID, LOAD_TS, SRC FROM (SELECT HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', CONCAT_WS(''|'', TOUCHPOINT_ID, ''int_surveyhero001''), CONCAT_WS(''|'', QUESTION_ID, ''int_surveyhero001''), CONCAT_WS(''|'', ANSWER_ID, ''int_surveyhero001'')) AS VARBINARY(MAX))) AS LNK_ID, HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', TOUCHPOINT_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) AS TOUCHPOINT_HUB_ID, HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', QUESTION_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) AS QUESTION_HUB_ID, HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', ANSWER_ID, ''int_surveyhero001'') AS VARBINARY(MAX))) AS ANSWER_HUB_ID, GETDATE() AS LOAD_TS, ''int_surveyhero001'' AS SRC, ROW_NUMBER() OVER (PARTITION BY HASHBYTES(''SHA2_256'', CAST(CONCAT_WS(''|'', CONCAT_WS(''|'', TOUCHPOINT_ID, ''int_surveyhero001''), CONCAT_WS(''|'', QUESTION_ID, ''int_surveyhero001''), CONCAT_WS(''|'', ANSWER_ID, ''int_surveyhero001'')) AS VARBINARY(MAX))) ORDER BY (SELECT NULL)) AS rn FROM stage.SH_MAIN) sub WHERE rn = 1', 1, N'Load', 0, N'Data Vault load step for ANSWER_QUESTION_TOUCHPOINT', NULL, 3, 30, N'[]', GETDATE(), GETDATE());
END
GO
