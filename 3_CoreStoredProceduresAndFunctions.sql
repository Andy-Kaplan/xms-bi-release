...
DROP FUNCTION IF EXISTS [core].[GetParameterWithType];
GO

CREATE FUNCTION [core].[GetParameterWithType]
        (
...
-- Drop if exists and recreate
DROP FUNCTION IF EXISTS [core].[GetParameter];
GO

CREATE FUNCTION [core].[GetParameter]
        (
            @ParameterKey nvarchar(100)
        )
...
-- Drop if exists and recreate
DROP FUNCTION IF EXISTS [core].[GetParameterDataType];
GO

CREATE FUNCTION [core].[GetParameterDataType]
        (
            @ParameterKey nvarchar(100)
        )
        RETURNS varchar(20)
...
-- Drop if exists and recreate
DROP FUNCTION IF EXISTS [core].[GetTypedParameter];
GO

CREATE FUNCTION [core].[GetTypedParameter]
        (
            @ParameterKey nvarchar(100),
...
-- Drop if exists and recreate
DROP FUNCTION IF EXISTS [core].[GetDatabaseFromOrganisationID];
GO

CREATE FUNCTION [core].[GetDatabaseFromOrganisationID]
        (
            @intID int
        )
        RETURNS nvarchar(4000)
...
-- Drop if exists and recreate
DROP FUNCTION IF EXISTS [core].[GetSchemaFromIntegrationID];
GO

CREATE FUNCTION [core].[GetSchemaFromIntegrationID]
        (
            @intID int
        )
        RETURNS nvarchar(4000)
...
-- Drop if exists and recreate
DROP FUNCTION IF EXISTS [core].[SHA256Hash];
GO

CREATE FUNCTION [core].[SHA256Hash](@input NVARCHAR(MAX))
...
CREATE OR ALTER PROCEDURE [core].[AddAPIEndpoint]
    @IntegrationID int,
...
CREATE OR ALTER PROCEDURE [core].[AddIntegration]
...
CREATE OR ALTER PROCEDURE [core].[AddOrganisation]
...
CREATE OR ALTER PROCEDURE [core].[CreateOrganisationDatabase] 
...
CREATE OR ALTER PROCEDURE [core].[CreatePendingDatabases]
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [core].[CreateDatabaseSchemas]
...
CREATE OR ALTER PROCEDURE [core].[CreateIntegrationSchema]
    @IntegrationID int
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [core].[GetIntegrationEndpoints]
    @IntegrationID int,
    @ActiveOnly bit = 1
AS
BEGIN
...
CREATE OR ALTER PROCEDURE [core].[GetIntegrations]
    @ActiveOnly bit = 1
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [core].[GetOrganisationIntegrations]
...
CREATE OR ALTER PROCEDURE [core].[GetOrganisations]
    @ActiveOnly bit = 1,
...
CREATE OR ALTER PROCEDURE [core].[MapOrganisationToIntegration]
    @OrganisationID int,
    @IntegrationID int,
...
CREATE OR ALTER PROCEDURE [core].[SetParameter]
            @ParameterKey nvarchar(100),
...
CREATE OR ALTER PROCEDURE [core].[SyncDatabaseStatus]
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [core].[UpdateOrganisationStatus]
    @OrganisationCode uniqueidentifier,
...
-- =============================================
CREATE OR ALTER PROCEDURE [core].[BuildColumnList]
...
BEGIN
    ...
    -- Build columns part with aliases from entity_columns (same join for both links and hubs)
    SELECT @columnsPart = STRING_AGG(
        CASE sc.hash_type
            WHEN 0 THEN sc.column_name
...
CREATE OR ALTER PROCEDURE [core].[ProcessSingleEntityMapping]
    @intSchema NVARCHAR(128),
    @entityName NVARCHAR(128),
...
CREATE OR ALTER PROCEDURE [core].[UploadEntityMappings]
    @intSchema NVARCHAR(128),
